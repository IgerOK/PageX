            ...
            if (content != "") {
                ...
                for section in sections {
                    ...
                    if (firstLine = "[SETTINGS]") {
                        ...
                        ; Загружаем данные вкладок (оставшаяся часть после первой строки)
                        tabData := SubStr(trimmedSection, StrLen("[TAB_DATA]") + 2)
                        if (tabData != "") {
                            ; Разделяем по разделителям <TAB-X<
                            ...
                            loop tabParts.Length {
                                ...
                                if (tabIndex = 1) {
                                    ; Первая вкладка (до первого разделителя)
                                    ...
                                    closePos := InStr(part, "<")
                                    if (closePos > 0) {
                                        ; Извлекаем содержимое после закрывающего <
                                        tabContent := SubStr(part, closePos + 1)
                                        TabContents[tabIndex] := Trim(tabContent, " `t`r`n")
                                    } else {
...
ShowHelp(*) {
    ...
    helpText .= "• Ctrl+F1 - Свернуть/развернуть окно (глобально)`n"  ; ← ИЗМЕНЕНО: удалено упоминание F2
