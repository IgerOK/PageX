...
global UndoHistory := Map()  ; История изменений для каждой вкладки
global RedoHistory := Map()  ; История для повтора изменений
global MaxUndoSteps := 100   ; Максимальное количество шагов отката
...
InitializeUndoHistory() {
    global UndoHistory, RedoHistory, TabCount
    
    loop TabCount {
        index := A_Index
        if (!UndoHistory.Has(index)) {
            UndoHistory[index] := []
        }
        if (!RedoHistory.Has(index)) {
            RedoHistory[index] := []
        }
    }
}

; === ДОБАВЛЕНИЕ СОСТОЯНИЯ В ИСТОРИЮ ОТКАТА ===
AddToUndoHistory(tabIndex, text) {
    global UndoHistory, RedoHistory, MaxUndoSteps
    
    ; Инициализируем историю для вкладки, если нужно
    if (!UndoHistory.Has(tabIndex)) {
        UndoHistory[tabIndex] := []
    }
    if (!RedoHistory.Has(tabIndex)) {
        RedoHistory[tabIndex] := []
    }
    
    ; Добавляем текущее состояние в историю отката
    undoArray := UndoHistory[tabIndex]
    undoArray.Push(text)
    
    ; Ограничиваем размер истории
    if (undoArray.Length > MaxUndoSteps) {
        undoArray.RemoveAt(1)
    }
    
    ; Очищаем историю повтора при новом изменении
    RedoHistory[tabIndex] := []  ; Исправлено: заменили Clear() на присвоение пустого массива
}

; === ОТКАТ ИЗМЕНЕНИЯ ===
UndoChange() {
    global CurrentTabIndex, UndoHistory, RedoHistory, TabContents, contentEdit
    
    ; Проверяем наличие истории отката для текущей вкладки
    if (!UndoHistory.Has(CurrentTabIndex) || UndoHistory[CurrentTabIndex].Length < 2) {
        return false  ; Недостаточно состояний для отката
    }
    
    ; Получаем текущий текст
    currentText := contentEdit.Value
    
    ; Получаем предыдущее состояние из истории отката
    undoArray := UndoHistory[CurrentTabIndex]
    previousText := undoArray.Pop()  ; Удаляем текущее состояние
    
    if (undoArray.Length > 0) {
        ; Получаем состояние для отката
        textToRestore := undoArray[undoArray.Length]
        
        ; Добавляем текущее состояние в историю повтора
        redoArray := RedoHistory[CurrentTabIndex]
        redoArray.Push(currentText)
        
        ; Применяем откат
        TabContents[CurrentTabIndex] := textToRestore
        contentEdit.Value := textToRestore
        ...
        ShowUndoMessage("Откат выполнен")
...
; === ПОВТОР ИЗМЕНЕНИЯ ===
RedoChange() {
    global CurrentTabIndex, RedoHistory, TabContents, contentEdit
    
    ; Проверяем наличие истории повтора для текущей вкладки
    if (!RedoHistory.Has(CurrentTabIndex) || RedoHistory[CurrentTabIndex].Length = 0) {
        return false  ; Нет состояний для повтора
    }
    
    ; Получаем текущий текст
    currentText := contentEdit.Value
    
    ; Получаем следующее состояние из истории повтора
    redoArray := RedoHistory[CurrentTabIndex]
    textToRestore := redoArray.Pop()
    
    ; Добавляем текущее состояние обратно в историю отката
    undoArray := UndoHistory[CurrentTabIndex]
    undoArray.Push(currentText)
    ...
    ; Показываем уведомление о повторе
    ShowUndoMessage("Повтор выполнен")
...
SaveCursorPosition() {
    global CurrentTabIndex, CursorPositions, contentEdit
...
; === ВСПЛЫВАЮЩЕЕ СООБЩЕНИЕ ОБ ОТКАТЕ/ПОВТОРЕ ===
ShowUndoMessage(message) {
    global Version
    
    undoMsg := Gui("+ToolWindow +AlwaysOnTop +Border", "PageX " Version)
    undoMsg.SetFont("s10", "Segoe UI")
    undoMsg.MarginX := 20
    undoMsg.MarginY := 15
    
    theme := GetCurrentTheme()
    undoMsg.BackColor := theme.Background
    
    msgText := undoMsg.Add("Text", "Center", "✓ " . message)
    msgText.SetFont("c" . theme.Text)
    msgText.Opt("Background" . theme.Background)
    
    undoMsg.Show("AutoSize Center NoActivate")
    
    SetTimer(() => CloseSaveMessage(undoMsg), 1500)
}
...
LoadData() {
    global DataFile, TabCount, TabContents
    global AlwaysOnTop, TransparencyLevel, FontSize, FontName, ThemeLevel, TabSize, CursorPositions, Fonts
...
MinimizeToTray() {
    ...
    try {
        ...
        TrayTip "PageX свёрнут в трей", "Клик по иконке или Ctrl+F1 для восстановления", "Info"
...
OnEditChangeWithLineNumbers(editCtrl, *) {
    ...
    ; Добавляем состояние в историю отката
    AddToUndoHistory(CurrentTabIndex, contentEdit.Value)
...
ShowHelp(*) {
    ...
    helpText .= "• Ctrl+Z  - Откат изменения`n"
    helpText .= "• Ctrl+Y  - Повтор изменения`n"
    ...
    helpText .= "↩️ ОТКАТ ИЗМЕНЕНИЙ:`n"
    helpText .= "• Поддерживается история до " MaxUndoSteps . " шагов`n"
    helpText .= "• Сохраняет состояния при каждом изменении текста`n"
    helpText .= "• Работает независимо для каждой вкладки`n"
...
#HotIf
^F1::ToggleWindowMode()  ; ← ОСТАВЛЕН ТОЛЬКО Ctrl+F1 (F2 удален)
...
LoadData()
InitializeUndoHistory()  ; Инициализация истории изменений
...
; Добавляем начальное состояние в историю отката
AddToUndoHistory(CurrentTabIndex, TabContents[CurrentTabIndex])
